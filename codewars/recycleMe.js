/*codewar challenge for the day link: https://www.codewars.com/kata/59fb783bab11f89202001083/train/javascript
7 kyu: Recycle
...
function recycleMe(recycle){ 
  ...
  let numOfPlastics = recycle.filter(x => x > 0).length
  let numOfGlass = recycle.filter(x => x < 0).length
  let numOfCard = recycle.filter(x => x == 0).length
  return [numOfPlastics, numOfGlass, numOfCard]
}