/*codewar challenge for the day link: https://www.codewars.com/kata/583df40bf30065fa9900010c/train/javascript
...
function getMean(arr, x, y) {
    if (x > 1 && y > 1 && x <= arr.length && y <= arr.length) {
      const firstMean = (arr.slice(0, x).reduce((a, b) => a + b, 0)/x)
      const secondMean = (arr.slice(arr.length - y, arr.length).reduce((a, b) => a + b, 0)/y)
      return (firstMean + secondMean) / 2
    } else {
      return -1
    }
  }