/*codewar challenge for the day link: https://www.codewars.com/kata/59619e4609868dd923000041/train/javascript
7 kyu: Sushi-go-round (Beginner's)
...
function totalBill(str) {
    let number = str.split("").filter(x => x == "r").length
    let freePlates = 0
    if (number >= 5) {
      freePlates += Math.floor(number/5)
    }
    return (number - freePlates) * 2
  }